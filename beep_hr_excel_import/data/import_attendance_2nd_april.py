...
def create_update_attendance(att_date, emp_code, punch_in, punch_out, remark):
    ...
    employee_id = odoo.env['hr.employee'].search(
        [('emp_code', '=like', '%'+attendance_id)])

    # Map Attendance ID with employee ID
    if employee_id:
        employee_rec = odoo.env['hr.employee'].browse(employee_id)
        if not (punch_in_utc and punch_out_utc) and remark in ["SL", "CL", "PL", "L"]:
            if remark == "SL":
                leave_type = odoo.env['hr.leave.type'].search(
...
file_location = os.path.join(
    "os.getcwd()", "bitbucket/UAT/erp-smnl/beep_hr_excel_import/data/import_attendance_2nd_april.xlsx")
book = xlrd.open_workbook(file_location)
print("Script Started with Database " + DB)
...
for i in range(sheet.nrows - 2):
    ...
    if sheet.cell_value(i + 2, 3):
        ...
        punch_out_time = date.replace(
            hour=punch_out[3]+12, minute=punch_out[4], second=punch_out[5])

    create_update_attendance(
        date, emp_code, punch_in_time, punch_out_time, remark)
