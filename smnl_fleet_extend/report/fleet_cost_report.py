...
class FleetCostReport(models.AbstractModel):
    _name = 'report.smnl_fleet_extend.report_fleet_cost'
    ...
    def _get_report_values(self, docids, data=None):
        ...
        if data.get('from_date') and data.get('to_date'):
            ...
            docs = self.env['fleet.vehicle.cost'].browse(data.get('ids'))
        else:
            docs = self.env['fleet.vehicle.cost'].browse(docids)

        return {
            'doc_ids': docs.ids,
            'doc_model': 'fleet.vehicle.cost',
            'data': data,
            'docs': docs,
            'from_date': from_date,
            'to_date': to_date,
